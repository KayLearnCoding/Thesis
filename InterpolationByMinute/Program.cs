...
namespace getCarID
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            DateTime start, end;
            start = DateTime.Now;
            //long SelectedLineNum = Convert.ToInt32(0.5*Math.Pow(10, 7));
            //string originPath = @"E:\origin_data\01";
            //string destFilePath = @"E:\testData\01\Record" + SelectedLineNum.ToString() + ".txt";
            //SelectLinesUntil(originPath,destFilePath, SelectedLineNum);

            for (int date = 1; date < 2; date++)
            {
                string dateStr = date.ToString(); if (date < 10) dateStr = "0" + dateStr;
                string originPath = @"E:\testData\01\R100w";

                string idFilePath = @"E:\testData\fullCarIDs_day" + dateStr + ".txt";
                string CarPath = @"E:\testData\" + dateStr + @"AllCar\";
                string InterpolateCarPath = @"E:\testData\" + dateStr + @"AllCarInterpolation\";
                ...
                //FindCarRecordBy(dateStr, time, @"D:\taxi_data\interpolation_data\" + dateStr + "AllCarInterpolation", @"D:\taxi_data\interpolation_data\day" + dateStr + ".txt");
            }
            end = DateTime.Now;
            Console.WriteLine("Cost Time: " + TimeDiff(start, end));
            Console.ReadKey();
        }

        private static void SelectLinesUntil(string originPath, string destFilePath, long selectedLineNum)
        {
            ...
            string[] names = Directory.GetFiles(originPath);
            ...
            for (int pathIndex = 0; pathIndex < names.Length; pathIndex++)
            {
                ...
                Console.WriteLine("正在处理txt：{0}", path);
                StreamReader sr = new StreamReader(path, Encoding.Default);
                string line;
                //读取第一遍原始数据
                while ((line = sr.ReadLine()) != null)
                {
                    if (recordNum < selectedLineNum)
                    {
                        lines.Add(line);
                        recordNum++;
                    }
        ...
        //Write one txt to record each day's car IDs return "get fullCarIDs_day02_11o'clock.txt"
        {
            ...
            // write a file for mysql
            FileStream fs = new FileStream(writePath, FileMode.Create); 
            StreamWriter sw = new StreamWriter(fs);
            FileStream fs2 = new FileStream(@"E:\all.csv", FileMode.Append); StreamWriter sw2 = new StreamWriter(fs2, Encoding.UTF8);
            long recordNum = 0;
            for (int pathIndex = 0; pathIndex < names.Length; pathIndex++)
            {
                string path = names[pathIndex];
            ...
            sw.Flush();
            //关闭流
            sw.Close();fs.Close(); sw2.Flush(); sw2.Close(); fs2.Close();
        ...
        static void updatedRecordsIntoCarIDTxts(string originalDataFolder, string writeFolder, string dateStr,string fullCarIDsTxtPath)
        {
            ...
            if (!Directory.Exists(writeFolder))                             // D:\interpolation_data\01AllCar\
                Directory.CreateDirectory(writeFolder);
